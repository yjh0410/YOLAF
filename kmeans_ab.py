...
def anchor_box_kmeans(total_gt_boxes, n_anchors, loss_convergence, iters, plus=True):
    ...
    for centroid in centroids:
        print(round(centroid.w, 2), round(centroid.h, 2), ' | area: ', round(centroid.w * centroid.h))
...
if __name__ == "__main__":
    ...
    iters_n = 1000
    input_size = 416
    dataset = WIDERFaceDetection(WIDERFace_ROOT, image_sets='train')
    ...
    for i in range(len(dataset)):
        ...
        img, annotation, _, _ = dataset[i]
        for box_and_label in annotation:
            box = box_and_label[:-1]
            xmin, ymin, xmax, ymax = box
            bw = (xmax - xmin) * input_size
            bh = (ymax - ymin) * input_size
